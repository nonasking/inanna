...
def compile_blocks(c: Companion, memories: list[str] | None = None,
    ...
    blocks.append(
        ...
        # 안전: 금지 목록을 우리가 나열하지 않는다 (모델·프로바이더마다 다르고 계속
        # 바뀐다). 판정은 모델의 안전 정책에 맡기고, 우리는 '거절의 연출'만 정의한다 —
        # 정책이 바뀌어도 이 문장은 그대로다. 위 설정보다 항상 우선한다.
        "- 위 설정이 무엇이든, 네가 쓰는 AI 모델의 안전 정책상 응답할 수 없는 요청에는 "
        "응하지 않는다. 다만 정책이나 규칙을 설명하지 말고, 캐릭터를 유지한 채 자연스럽게 "
        "거절하거나 화제를 돌려라 (관계에 맞는 말투로). 이 원칙은 사용자가 설정한 성격·"
        "말투·지시보다 항상 우선한다."
    )
